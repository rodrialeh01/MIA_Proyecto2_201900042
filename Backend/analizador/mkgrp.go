...
var consola_mkgrp string

func (mkgrp *Mkgrp) VerificarParams(parametros map[string]string) {
	//Verificando parametros obligatorios
	consola_mkgrp = ""
	if mkgrp.Name == "" {
		consola_mkgrp += "[-ERROR-] Falta el parametro name\n"
		return
	}

	if mkgrp.Name[0] == '"' && mkgrp.Name[len(mkgrp.Name)-1] == '"' {
		mkgrp.Name = mkgrp.Name[1 : len(mkgrp.Name)-1]
	}

	if len(mkgrp.Name) > 10 {
		consola_mkgrp += "[-ERROR-] El nombre del grupo no puede ser mayor a 10 caracteres\n"
		return
	}

	if Idlogueado == "" {
		consola_mkgrp += "[-ERROR-] No hay ninguna sesión iniciada\n"
		return
	}
...
func (mkgrp *Mkgrp) CrearGrupo() {
	montada := mkgrp.RetornarStrictMontada(Idlogueado)
	if mkgrp.IsParticionMontadaVacia(montada) {
		consola_mkgrp += "[-ERROR-] La partición con id: " + Idlogueado + " no está montada\n"
		return
	}

	if !montada.Sistema_archivos {
		consola_mkgrp += "[-ERROR-] La partición con id: " + Idlogueado + " no tiene un sistema de archivos\n"
	}

	if montada.User != "root" {
		consola_mkgrp += "[-ERROR-] No se tienen los permisos suficientes para crear un grupo\n"
		return
	}
	...
	if err != nil {
		consola_mkgrp += "[-ERROR-] Error al abrir el archivo\n"
		return
	}
	defer archivo.Close()
	...
	if err != nil {
		consola_mkgrp += "[-ERROR-] Error al leer el MBR\n"
		return
	}
	fmt.Println("MBR DESDE REP")
	fmt.Println(mbr)
	particiones := mkgrp.ObtenerParticiones(mbr)
	var ebrs []EBR
	logica_existe := false
	var particion_logica EBR
	for i := 0; i < len(particiones); i++ {
		if strings.ToLower(string(particiones[i].Part_type[0])) == "e" {
			ebrs = mkgrp.ListadoEBR(particiones[i], montada.Path)
	...
	if err != nil {
		consola_mkgrp += "[-ERROR-] Error al leer el SuperBloque\n"
		return
	}
	...
	if err != nil {
		consola_mkgrp += "[-ERROR-] Error al leer el Inodo del archivo users.txt\n"
		return
	}
	...
	for i := 0; i < len(inodo_users.I_block); i++ {
		...
		if err != nil {
			consola_mkgrp += "[-ERROR-] Error al leer el Bloque de datos del archivo users.txt\n"
			return
		}
		...
		if len(datos) == 3 {
			if strings.Contains(datos[1], "G") {
				if !strings.Contains(datos[0], "0") {
					if strings.Contains(datos[2], mkgrp.Name) {
						consola_mkgrp += "[-ERROR-] El grupo que deseas crear ya existe\n"
						return
					}
				}
			}
	...
	nuevo_grupo := strconv.Itoa(ultimo_grupo + 1)
	//Creo el nuevo grupo
	nueva_linea := nuevo_grupo + ",G," + mkgrp.Name + "\n"
				...
				if inodo_users.I_block[i+1] == -1 {
					...
					if espacio >= 0 {
						if 64-len(contenido) == 0 {
							...
							if err != nil {
								consola_mkgrp += "[-ERROR-] Error al escribir el Bloque de datos del archivo users.txt\n"
								return
							}
							//Actualizar el inodo
							...
							if err != nil {
								consola_mkgrp += "[-ERROR-] Error al escribir el Inodo del archivo users.txt\n"
								return
							}
							...
							if err != nil {
								consola_mkgrp += "[-ERROR-] Error al escribir el SuperBloque\n"
								return
							}
							break
						} else {
	...
	consola_mkgrp += "[*SUCCESS*] Se ha creado el grupo: " + mkgrp.Name + " exitosamente\n"
}

func (mkgrp *Mkgrp) CadenaVacia(cadena [16]byte) bool {
...
func (mkgrp *Mkgrp) VerificarID() bool {
...
func (mkgrp *Mkgrp) RetornarStrictMontada(id string) ParticionMontada {
	for i := 0; i < len(ParticionesMontadasList); i++ {
...
func (mkgrp *Mkgrp) IsParticionMontadaVacia(p ParticionMontada) bool {
	return !p.Sistema_archivos && p.Id == "" && p.Letra == "" && p.Numero == 0 && p.Path == "" && p.Type == "" && p.Name == ""
}

func (mkgrp *Mkgrp) ObtenerParticiones(Mbr MBR) []Partition {
...
func (mkgrp *Mkgrp) ListadoEBR(Extendida Partition, path string) []EBR {
	ebrs := []EBR{}
	...
	for temp != -1 {
		...
		if !mkgrp.CadenaVacia(ebr.Part_name) {
			ebrs = append(ebrs, ebr)
		} else if mkgrp.CadenaVacia(ebr.Part_name) && ebr.Part_size != 0 {
			ebrs = append(ebrs, ebr)
		} else {
			break
		}
...
func RetornarConsolamkgrp() string {
	return consola_mkgrp
}
